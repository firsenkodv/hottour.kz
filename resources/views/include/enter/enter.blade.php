<div class="enter_to_website">

    @auth

<div class="enter_to_website__a">
        <a href="{{ route('setting') }}" class="">

            <div class="site_avatar" style="background-image: url('@if(isset(auth()->user()->avatar)) {{ Storage::disk('user')->url(auth()->user()->avatar) }} @else {!! 'data:image/svg+xml;base64,' !!} @endif ');  width: 36px; height: 36px"></div>



        </a>
        <x-forms.auth-form
            title=""
            subtitle=""
            action="{{ route('logout') }}"
            method="POST"
        >
            <button type="submit" class="enter_to_website__a2"> <span class="sp__kab">{{__('Выход')}}</span> </button>
        </x-forms.auth-form>
</div>


    @endauth

    @guest
            <a href="/login" class="enter_to_website__a">
                <div class="site_avatar" style="background-image: url('data:image/svg+xml;base64,');  width: 36px; height: 36px"></div>
                <span class="sp__kab">{{__('Вход')}}</span>
            </a>
    @endguest



</div>

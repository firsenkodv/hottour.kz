<div class="dashboardBox">
    ...
    <div class="dashboardBox__a_users a_users pad_b26">
        ...
        @foreach($managers as $manager)
            {{--    Тут выводлим только  закрепленных--}}
            @if(isset($manager->fixed_manager->id))
                @if($manager->fixed_manager->id == $item->id )
                    <div
                        class="a_user__row a_user @if(!$manager->published) published_0 @endif  @if(role($manager->id) == 'manager' or role($manager->id) == 'senior') background_green @endif @if(role($manager->id) == 'admin') background_biruza @endif">
                        <div class="a_user__checkbox">


                            <input class="checkbox-flip checkbox_change" data-checkbox="{{ $manager->id }}"
                                   value="{{ $manager->id }}" type="checkbox" id="check_{{$manager->id}}"/>
                        ...
                        <div class="a_user__name">
                            ...
                            <div
                                class="a_user__nameBirthdate color_grey color_grey_12">{{ ($manager->birthdate)? birthdate($manager->birthdate) :'' }}</div>
        ...
        <div class="checkbox__js">
            <x-forms.form
                title=""
                subtitle=""
                action="{{ route('update.managers_minus.senior') }}"
                method="POST"
            >
                ...
                <div class="chose_filter__button">
                    <div class="text_input">
                        ...
                        <x-forms.primary-button>
                            {{ __('Удалить из команды  РОП -') }} {{ $item->name }}
                        </x-forms.primary-button>
                    </div>
                </div>
